...
def SES_model(data, set_horizon, alpha_high, alpha_low):
    ...
    fore_high = res_high.forecast(set_horizon)
    ...
    fore_low = res_low.forecast(set_horizon)
...
def Holt_model(data,set_horizon, level_high, level_low,trend_high,trend_low):
...
def Holt_Winter_Model(data,set_horizon, level_high, level_low,trend_high,trend_low,season_high,season_low):
...
def AR_model(data,set_horizon, p_high,p_low):
...
def MA_model(data,set_horizon, q_high,q_low):
    from statsmodels.tsa.arima.model import ARIMA
...
def ARMA_model(data,set_horizon,p_high,p_low, q_high,q_low):
    from statsmodels.tsa.arima.model import ARIMA
...
def ARIMA_model(data,set_horizon,p_high,p_low,q_high,q_low,i_high,i_low):
    from statsmodels.tsa.arima.model import ARIMA
...
def Auto_Arima(data,set_horizon):
    from pmdarima import auto_arima
    index = pd.bdate_range(start=data.index[-1], periods=(set_horizon+1))
    model_high = auto_arima(data['High'])
    fore_high = model_high.predict(set_horizon)
    fore_high = np.insert(fore_high, 0, data['High'][-1])
    fore_high = pd.DataFrame(fore_high, index=index)
    fore_high.columns = ['Forecast_High']

    model_low = auto_arima(data['Low'])
    fore_low = model_low.predict(set_horizon)
